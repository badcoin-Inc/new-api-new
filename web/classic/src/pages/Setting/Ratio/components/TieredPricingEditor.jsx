...
function evalExprLocally(exprStr, p, c, extraTokenValues) {
  try {
    ...
    const env = {
      ...
      len,
      tier: tierFn,
      param: (path) => localParamValue(path),
      header: () => '',
      has: (source, substr) =>
        source != null && String(source).includes(substr),
      hour: (tz) => localDatePart(tz, 'hour'),
      minute: (tz) => localDatePart(tz, 'minute'),
      weekday: (tz) => localDatePart(tz, 'weekday'),
      month: (tz) => localDatePart(tz, 'month'),
      day: (tz) => localDatePart(tz, 'day'),
      max: Math.max,
      min: Math.min,
      abs: Math.abs,
      ceil: Math.ceil,
...
function localParamValue(path) {
  if (path === 'billing_size') return '2K';
  return null;
}

function localDatePart(tz, part) {
  const date = tz
