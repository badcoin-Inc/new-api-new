...
export default function DynamicPricingBreakdown({ billingExpr, t }) {
  ...
  const priceFields = [BILLING_FIXED_PRICE_VAR, ...BILLING_PRICING_VARS].map(
    (v) => [v.field, v.shortLabel, v.field === 'fixedPrice'],
  );
  ...
  const tierColumns = [
    ...
    ...priceFields
      ...
      .map(([field, label, isFixedPrice]) => ({
        title: `${t(label)} (${symbol}/${isFixedPrice ? t('次') : '1M tokens'})`,
        dataIndex: field,
        render: (v) =>
          v > 0 ? (
            <Text
              strong
            >{`${symbol}${((isFixedPrice ? v / 1000000 : v) * rate).toFixed(4)}`}</Text>
          ) : (
            '-'
          ),
      })),
