...
// 获取侧边栏Lucide图标组件
export function getLucideIcon(key, selected = false, selectedColor) {
  const size = 16;
  const strokeWidth = 2;
  const SELECTED_COLOR = selectedColor || 'var(--semi-color-primary)';
...
export function renderTieredModelPrice(opts) {
  ...
  const priceLines = [BILLING_FIXED_PRICE_VAR, ...BILLING_PRICING_VARS]
    .filter((v) => v.group !== 'cache' || hasAnyCacheTokens)
    .map((v) => [v.field, v.label, v.field === 'fixedPrice']);

  const lines = [
    buildBillingText('命中档位：{{tier}}', { tier: matchedTier || tier.label }),
    ...priceLines
      .filter(([field]) => tier[field] > 0)
      .map(([field, label, isFixedPrice]) =>
        buildBillingPriceText(
          `${label}：{{symbol}}{{price}} / ${isFixedPrice ? '次' : '1M tokens'}`,
          {
            symbol,
            usdAmount: isFixedPrice ? tier[field] / 1000000 : tier[field],
            rate,
          },
        ),
      ),
  ...
  if (outputMode === 'segments') {
    ...
    if (!tier) {
      ...
      const hasAnyCacheTokens =
        ...
        cacheCreationTokens5m > 0 ||
        cacheCreationTokens1h > 0;
      const priceSegments = [BILLING_FIXED_PRICE_VAR, ...BILLING_PRICING_VARS]
        .filter((v) => v.group !== 'cache' || hasAnyCacheTokens)
        .map((v) => [v.field, v.shortLabel, v.field === 'fixedPrice']);
      for (const [field, label, isFixedPrice] of priceSegments) {
        if (tier[field] > 0) {
          segments.push({
            tone: 'secondary',
            text: i18next.t('{{label}} {{price}} / {{unit}}', {
              label: i18next.t(label),
              price: formatCompactDisplayPrice(
                isFixedPrice ? tier[field] / 1000000 : tier[field],
              ),
              unit: isFixedPrice ? i18next.t('次') : '1M tokens',
            }),
          });
        }
      }
